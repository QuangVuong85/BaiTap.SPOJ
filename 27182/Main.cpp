...
int checkBlocks(int x, int y) {
    ...
    for(int i = startx*4; i < end_x; i++)
        for(int j = starty*4; j < end_y; j++) {
            ...
            F[matrix[i][j]]++;
            if(F[matrix[i][j]] > 2)	return 1;
        }
        
    return 0;
}

int checkRowCol(int index_x, int index_y) {
    int F_row[10];
    int F_col[10];
    ...
    for(int test = 1; test <= n; test++) {
        ...
        for(int i = 0; i < 8; i++)
            ...
            if(!checkBlocks(0, 0) && 
                ...
                else cout << "Test case #" << test << ": NO" << endl;
            }
            else cout << "Test case #" << test << ": NO" << endl;
    }
